// app/(jinx)/layout.tsx
// Purple-themed layout for Jinx Mode. Guards: user must have role=jinx
// and current_mode=jinx. Otherwise redirects to /home.
'use client'
...
export default function JinxLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (profile.role !== 'jinx') { router.replace('/home'); return }
    if (profile.current_mode !== 'jinx') { router.replace('/home'); return }
  }, [profile, loading])

  if (loading || !profile || profile.role !== 'jinx' || profile.current_mode !== 'jinx') {
    return (
