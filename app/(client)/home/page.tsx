...
export default function HomePage() {
  ...
  const [displayName, setDisplayName] = useState<string | null>(null) // null = loading
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      if (profile) {
        ...
        setDisplayName('') // fallback — profile missing
      }
    })
  ...
  // Cycle greeting lines every 20 minutes — only after name loads
  useEffect(() => {
    ...
    <div className="relative w-full" style={{ minHeight: '100dvh' }}>
              ...
              >
                <div className="w-14 h-14 rounded-full p-0.5" style={{
                  ...
                  <div className="w-full h-full rounded-full flex items-center justify-center" style={{
                    ...
                    <span className="text-sm font-medium text-white">{story.username[0]}</span>
                  </div>
                </div>
      ...
      >
        <div className="mb-4">
          ...
          {/* Greeting line — shimmer while loading, real text once ready */}
          {currentLine === null ? (
            // Skeleton shimmer — two lines of varying width
