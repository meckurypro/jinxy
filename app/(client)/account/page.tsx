...
// Role-aware: clients see "Become a Jinx", jinx users see mode toggle.
'use client'
...
interface Stats { favourites: number; likes: number; jinxes: number }
interface AvatarMoment { id: string; storage_path: string }
...
function Shimmer({ width, height, rounded = 8 }: { width: string | number; height: number; rounded?: number }) {
  return (
    <div style={{
      ...
      animation: 'skeleton-pulse 1.5s ease-in-out infinite', flexShrink: 0,
    }} />
  )
}
...
export default function AccountPage() {
  ...
  const fetchStats = async () => {
    ...
    setStats({ favourites: favsRes.count ?? 0, likes: likesRes.count ?? 0, jinxes: jinxesRes.count ?? 0 })
  }

  const fetchAvatarMoment = async () => {
    if (!profile?.id) return
    const { data } = await supabase.from('media').select('id, storage_path')
      .eq('user_id', profile.id).eq('category', 'moment').eq('is_active', true).eq('is_avatar', true).maybeSingle()
  ...
  // Switch from client mode to jinx mode (only for role=jinx users)
  const handleSwitchToJinxMode = async () => {
    if (!profile?.id || togglingMode) return
    setTogglingMode(true)
    const { error } = await supabase.from('users').update({ current_mode: 'jinx' }).eq('id', profile.id)
    if (!error) {
      await refresh()
  ...
  // Role-aware Jinx section
  const jinxSection = profile?.role === 'client'
    ? {
        title: 'Earn',
        items: [
          {
    ...
    : {
        ...
        items: [
          {
            label: togglingMode ? 'Switching...' : 'Switch to Jinx Mode',
            icon: '💜',
            action: handleSwitchToJinxMode,
            highlight: true,
    ...
    <div className="min-h-dvh" style={{ background: 'var(--bg-base)' }}>
      ...
      <div className="relative px-5 pt-14 pb-6">
        <div className="flex items-center gap-4 mb-5">
          {isProfileLoading ? (
            <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'rgba(255,255,255,0.06)', animation: 'skeleton-pulse 1.5s ease-in-out infinite', flexShrink: 0 }} />
          ) : (
        ...
        {!isProfileLoading && (profile?.jinxy_credits ?? 0) > 0 && (
          <button onClick={() => router.push('/account/referrals')}
            ...
            <div className="flex items-center gap-2">
              <span>💰</span>
              <p className="text-sm font-medium" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-body)' }}>Jinxy Credits</p>
            </div>
...
// ─── AvatarViewer (same as before — kept inline) ──────────────────────────────
...
interface ImageMoment { id: string; storage_path: string; is_avatar: boolean }
...
function AvatarViewer({ userId, currentAvatarMoment, supabase, onClose, onChanged }: AvatarViewerProps) {
  ...
  const fetchImageMoments = async () => {
    ...
    const { data } = await supabase.from('media').select('id, storage_path, is_avatar')
      .eq('user_id', userId).eq('category', 'moment').eq('media_type', 'image').eq('is_active', true)
      .order('display_order', { ascending: true })
    if (data) setImageMoments(data as ImageMoment[])
    setLoadingMoments(false)
  }

  useEffect(() => { if (mode === 'pick') fetchImageMoments() }, [mode])
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: '#000' }}>
      ...
      {mode === 'pick' && (
        <>
          <div className="flex items-center justify-between px-5 pt-14 pb-4">
            <button onClick={() => setMode('view')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'white' }}>
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                <path d="M12 4L6 10L12 16" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
          ...
          <div className="flex-1 overflow-y-auto px-5">
            {loadingMoments ? (
              <div className="grid grid-cols-3 gap-2">
                {[1,2,3].map(i => <div key={i} style={{ aspectRatio: '1', borderRadius: 12, background: 'rgba(255,255,255,0.08)', animation: 'skeleton-pulse 1.5s ease-in-out infinite' }} />)}
              </div>
              ...
              <>
                <div className="grid grid-cols-3 gap-2 pb-6">
                  {imageMoments.map(moment => {
                    ...
                    return (
                      <button key={moment.id} onClick={() => handleSetAvatar(moment)}
                        ...
                        style={{ aspectRatio: '1', borderRadius: 12, overflow: 'hidden', position: 'relative',
                          ...
                          cursor: settingDp ? 'not-allowed' : 'pointer', padding: 0,
                          opacity: settingDp && !isSetting ? 0.5 : 1, transition: 'opacity 200ms ease' }}>
                        <img src={moment.storage_path} alt="Moment"
