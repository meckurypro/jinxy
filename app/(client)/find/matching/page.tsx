...
export default function MatchingPage() {
  ...
  useEffect(() => {
        ...
        (payload) => {
          if (payload.new && (payload.new as Record<string, unknown>).status === 'accepted') {
            setAcceptedCount(prev => {
              ...
              if (newCount >= 2) {
                // Enough matches — navigate to matches screen
                clearInterval(intervalRef.current!)
                setProgress(100)
                setPhase('found')
                setTimeout(() => {
                  router.replace(`/find/matches?booking=${bookingId}`)
                }, 1000)
              }
              return newCount
            })
    ...
    // After 3 minutes, show whatever we have
    const timeout = setTimeout(async () => {
      ...
      if (responses && responses.length >= 1) {
        ...
        // No matches — go back with feedback
        router.replace(`/find?error=no_matches`)
      }
    }, 3 * 60 * 1000)

    // Expand phase after 30s
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="relative mb-8">
          ...
          >
            <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
              <path
                d="M18 32S4 23 4 13a8 8 0 0116 0 8 8 0 0116 0C36 23 18 32 18 32z"
        ...
        {/* Progress bar */}
        <div
          className="w-48 h-1 rounded-full mb-4 overflow-hidden"
        ...
        {/* Phase text */}
        <h2
          className="font-display text-2xl text-center mb-3 px-8"
        ...
        {acceptedCount > 0 && (
          <p
            ...
            style={{ color: 'rgba(0,217,126,0.9)', fontFamily: 'var(--font-body)' }}
          >
        ...
        {declineReasons.length > 0 && (
          <div
            className="mt-2 px-4 py-2 rounded-xl text-center"
            style={{
        ...
        {/* Warning */}
        <p
          className="text-xs mt-8 text-center px-8"
          style={{ color: 'rgba(255,255,255,0.2)', fontFamily: 'var(--font-body)' }}
        >
          Please don't close or minimize this window
          <br />while we find your match.
        </p>
      </div>
