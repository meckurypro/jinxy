...
export default function PaymentPage() {
  ...
  const fetchBooking = async () => {
    ...
    if (data) {
      ...
      // Calculate max credits usable (50%)
      const maxCredits = Math.floor((data.total_charged as number) * 0.5)
      const availableCredits = profile?.jinxy_credits ?? 0
      const usable = Math.min(maxCredits, availableCredits)
      setCreditsToUse(usable)
    }
  ...
  const actualCredits = useCredits ? Math.min(creditsToUse, maxCreditsUsable) : 0
  ...
  const handlePay = async () => {
    ...
    if (cashAmount > 0 && window.PaystackPop) {
      const handler = window.PaystackPop.setup({
        ...
        email: profile.email,
        amount: cashAmount * 100, // Paystack uses kobo
        currency: 'NGN',
        reference,
        metadata: {
        ...
        callback: async (response: { reference: string }) => {
          // Payment successful — record and confirm
        ...
        >
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <p className="text-sm font-medium" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-body)' }}>
                {formatCurrency((booking?.agreed_rate as number) ?? 0)}
              </p>
            </div>
            <div className="flex justify-between">
              <p className="text-sm" style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-body)' }}>
              ...
              <p className="text-sm font-medium" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-body)' }}>
                {formatCurrency((booking?.transport_fare as number) ?? 0)}
              </p>
            </div>
            <div
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-body)' }}>
                  {formatCurrency(profile?.jinxy_credits ?? 0)} available · saves {formatCurrency(Math.min(creditsToUse, maxCreditsUsable))}
                </p>
              </div>
